...
som_input_t calculate_features(raw_data_t history[], uint16_t window_size)
{
    ...
    signal_data_t signal_data = peak_detector(history, window_size);
    if (debug_zero_cross)
        ESP_LOGI(TAG, "Zerocrossings found: %u", signal_data.zero_crossings);
    ...
    ESP_LOGI(TAG, "Heartbeats: %.1f RMSSD: %.1f SDNN: %.1f", pulse_data.avg_hr, pulse_data.rmssd,
             pulse_data.sdnn);
    return features;
}
...
static heart_beat_stats_t calculate_rr_intervals(signal_data_t *data)
{
    ...
    for (int peak = 1; peak < data->peaks_count; peak++) {
        ...
        if (peak > 1) {
            int16_t diff = pulse_data.rr_intervals[peak - 1] - pulse_data.rr_intervals[peak - 2];
            diff *= diff;
