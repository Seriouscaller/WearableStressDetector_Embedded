...
void imu_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (bmi160_read(bmi_handle, &imu_data) == ESP_OK)
        {
            ...
            {
                ESP_LOGE(TAG, "Could not take mutex to read sensor data for IMU.");
            }
        }
        else
        {
...
void temp_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (tmp117_read_temp(tmp_handle, &current_temp) == ESP_OK)
        {
            ...
            {
                ESP_LOGE(TAG, "Could not take mutex to read sensor data for Temperature.");
            }
        }
...
void ppg_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (max30101_read_fifo(max_handle, &current_ppg) == ESP_OK)
        {
            ...
            {
                ESP_LOGE(TAG, "Could not take mutex to read sensor data for PPG.");
            }
        }
        else
        {
...
void gsr_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (gsr_sensor_read_raw(gsr_handle, &current_gsr) == ESP_OK)
        {
            ...
            {
                ESP_LOGE(TAG, "Could not take mutex to read sensor data for GSR.");
            }
        }
        else
        {
