#include "bmi260.h"
#include "bmi260_config.h" // Contains the 8KB bmi260_config_file array
#include "board_config.h"
#include "esp_log.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "i2c_common.h"

static esp_err_t upload_bmi260_config(i2c_master_dev_handle_t *dev_handle);

static const char *TAG = "BMI260";

#define BMI260_ADDR 0x68
#define BMI260_CHIP_ID_VAL 0x27
#define BMI260_REG_CHIP_ID 0x00
#define BMI260_REG_INTERNAL_STAT 0x21
#define BMI260_REG_INIT_CTRL 0x59
#define BMI260_REG_INIT_DATA 0x5E
#define BMI260_REG_PWR_CONF 0x7C
#define BMI260_REG_PWR_CTRL 0x7D
#define BMI260_REG_CMD 0x7E
#define BMI260_REG_DATA_ACC_X 0x0C
#define BMI260_REG_DATA_GYR_X 0x12
#define BMI260_REG_ACC_CONF 0x40
#define BMI260_REG_ACC_RANGE 0x41
#define BMI260_REG_GYR_CONF 0x42
#define BMI260_REG_GYR_RANGE 0x43
#define BMI260_INIT_START 0x00
#define BMI260_INIT_FINALIZE 0x01
#define BMI260_INIT_SUCCESS 0x01
#define BMI260_CMD_SOFTRESET 0xB6
#define BMI260_CMD_PWRSAVE_DISABLE 0x00

#define BMI260_ACC_RANGE_2G 0x03
#define BMI260_ACC_CONF_100HZ 0x28
#define BMI260_GYR_RANGE_2000 0x00
#define BMI260_GYR_CONF_100HZ 0x28

// Power Control Bits
#define BMI260_PWR_GYR_EN (1 << 1)
#define BMI260_PWR_ACC_EN (1 << 2)
#define BMI260_PWR_TEMP_EN (1 << 3)

/**
 * @brief Initializes the BMI260 IMU sensor via the I2C master bus.
 *
 * The BMI260 initialization process is more involved than its predecessors,
 * requiring a soft reset followed by a configuration file upload (microcode).
 * This function performs the hardware handshake, uploads the binary config,
 * verifies the internal status, and configures the sampling rates and ranges.
 *
 * @param[in]  bus_handle  The handle for the initialized I2C master bus.
 * @param[out] dev_handle  Pointer to the I2C device handle created for the BMI260.
 *
 * @return
 *      - ESP_OK:   Sensor is initialized, microcode uploaded, and ready for data.
 *      - ESP_FAIL: Hardware ID mismatch, config upload failure, or status check failed.
 */
esp_err_t bmi260_init(i2c_master_bus_handle_t bus_handle, i2c_master_dev_handle_t *dev_handle)
{
    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = BMI260_ADDR,
        .scl_speed_hz = I2C_MASTER_FREQ_HZ,
    };

    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, dev_handle));

    // Soft Reset
    write_reg(*dev_handle, BMI260_REG_CMD, BMI260_CMD_SOFTRESET);
    vTaskDelay(pdMS_TO_TICKS(10));

    // Disable advanced power saving
    write_reg(*dev_handle, BMI260_REG_PWR_CONF, 0x00);
    write_reg(*dev_handle, BMI260_REG_PWR_CTRL, 0x00);
    vTaskDelay(pdMS_TO_TICKS(10));

    uint8_t chip_id = 0;
    uint8_t reg = BMI260_REG_CHIP_ID;
    esp_err_t ret = i2c_master_transmit_receive(*dev_handle, &reg, 1, &chip_id, 1, -1);
    // Verify read success and correct chip ID (Expecting BMI260_CHIP_ID_VAL (0x27) for BMI260)
    if (ret != ESP_OK || chip_id != BMI260_CHIP_ID_VAL) {
        ESP_LOGE(TAG, "Read failed or Wrong ID. Got: 0x%02X, Err: %d", chip_id, ret);
        return ESP_FAIL;
    }

    // Upload config file to sensor
    if (upload_bmi260_config(dev_handle) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to upload config file");
        return ESP_FAIL;
    }
    vTaskDelay(pdMS_TO_TICKS(150)); // Wait for initialization

    // Check if the BMI is ready after uploading config file
    uint8_t status = 0;
    reg = BMI260_REG_INTERNAL_STAT;
    i2c_master_transmit_receive(*dev_handle, &reg, 1, &status, 1, -1);
    if ((status & 0x0F) != BMI260_INIT_SUCCESS) {
        ESP_LOGE(TAG, "Initialization failed. Status: 0x%02X", status);
        return ESP_FAIL;
    }

    // Accel: Sampling 100Hz, Normal modes
    write_reg(*dev_handle, BMI260_REG_ACC_CONF, BMI260_ACC_CONF_100HZ);
    // Gyro: Sampling 100Hz, Normal mode
    write_reg(*dev_handle, BMI260_REG_GYR_CONF, BMI260_GYR_CONF_100HZ);

    // Accelerometer range +-2g
    write_reg(*dev_handle, BMI260_REG_ACC_RANGE, BMI260_ACC_RANGE_2G);
    // Gyroscope range +- 2000 deg/s
    write_reg(*dev_handle, BMI260_REG_GYR_RANGE, BMI260_GYR_RANGE_2000);

    // Enable Accel and Gyro
    write_reg(*dev_handle, BMI260_REG_PWR_CTRL, BMI260_PWR_ACC_EN | BMI260_PWR_GYR_EN);
    vTaskDelay(pdMS_TO_TICKS(50));

    ESP_LOGI(TAG, "BMI260 Initialized successfully");
    return ESP_OK;
}

/**
 * @brief  Uploads the mandatory configuration microcode to the BMI260 ASIC.
 *
 * The BMI260 requires a binary configuration file to be uploaded to the
 * BMI260_REG_INIT_DATA register before any sensor data can be generated.
 * This function handles the memory allocation for the burst buffer,
 * prepends the register address, and transmits the entire file in one transaction.
 *
 * @param[in] dev_handle Pointer to the I2C device handle for the BMI260.
 *
 * @return
 *      - ESP_OK:   Configuration uploaded successfully.
 *      - ESP_FAIL: Memory allocation failed or I2C transmission timed out.
 *
 * @note The config file is a static array (bmi260_config_file) usually defined
 *       in a separate header file.
 */
static esp_err_t upload_bmi260_config(i2c_master_dev_handle_t *dev_handle)
{
    // Prepare for config upload
    write_reg(*dev_handle, BMI260_REG_INIT_CTRL, BMI260_INIT_START);

    // Upload Config File (Burst write)
    const size_t config_size = sizeof(bmi260_config_file);
    uint8_t *burst_buf = malloc(config_size + 1);

    if (burst_buf == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for config upload");
        return ESP_FAIL;
    }

    burst_buf[0] = BMI260_REG_INIT_DATA;
    memcpy(&burst_buf[1], bmi260_config_file, config_size);

    ESP_LOGI(TAG, "Uploading configuration (%d bytes)...", config_size);
    esp_err_t ret = i2c_master_transmit(*dev_handle, burst_buf, config_size + 1, pdMS_TO_TICKS(100));
    free(burst_buf);

    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Config upload failed. Err: %d", ret);
        return ret;
    }

    // Finalize config upload
    write_reg(*dev_handle, BMI260_REG_INIT_CTRL, BMI260_INIT_FINALIZE);
    return ESP_OK;
}

/**
 * @brief Reads raw 6-axis motion data from the BMI260.
 *
 * Performs a single I2C burst transaction to read 12 consecutive bytes starting
 * from the Accelerometer X-axis LSB register. This ensures that the motion
 * data from all axes is sampled from the same internal sensor update cycle.
 *
 * @param[in]  dev_handle I2C device handle for the BMI260.
 * @param[out] data       Pointer to a 'bmi_data_t' struct to hold the parsed results.
 *
 * @return
 *      - ESP_OK: Data successfully read and converted.
 *      - ESP_FAIL/ESP_ERR_TIMEOUT: I2C communication error.
 *
 * @note The BMI260 uses a little-endian format (LSB first).
 */
esp_err_t bmi260_read(i2c_master_dev_handle_t dev_handle, bmi_data_t *data)
{
    uint8_t reg = BMI260_REG_DATA_ACC_X; // Start at Accel registers
    uint8_t buf[12];
    uint8_t buf_size = sizeof(buf) / sizeof(buf[0]);

    // Read 12 bytes: 6 for Accel, 6 for Gyro
    esp_err_t ret = i2c_master_transmit_receive(dev_handle, &reg, 1, buf, buf_size, pdTICKS_TO_MS(100));

    if (ret == ESP_OK) {
        data->acc_x = (int16_t)((buf[1] << 8) | buf[0]);
        data->acc_y = (int16_t)((buf[3] << 8) | buf[2]);
        data->acc_z = (int16_t)((buf[5] << 8) | buf[4]);

        data->gyr_x = (int16_t)((buf[7] << 8) | buf[6]);
        data->gyr_y = (int16_t)((buf[9] << 8) | buf[8]);
        data->gyr_z = (int16_t)((buf[11] << 8) | buf[10]);
    } else {
        ESP_LOGW(TAG, "Failed to read IMU");
    }

    return ret;
}
